---
title: "Get started - Use renv package for dependency management in R"
date: "2022-10-29"
categories: [R, renv, Dependency management]
...
To show how `renv` actually works, we create a new RStudio project with `File > New Project ... > New directory`

![Fig.1: Create new project](newproject.PNG){fig-align="center"}

As you can see, after installing `renv`, a dedicated tickbox is added to new projects windows. Clicking on it will automatically initialize `renv` when creating the project. Here we leave it unchecked, to show how to manually initialize the dependency management. We call the new project 'renv-test-project'. Below you can see its main directory.

![Fig 2: New project main dir](main_dir1.PNG){fig-align="center"}
...
![Fig. 3: Main dir after renv initialization](main_dir2.PNG){fig-align="center"}

As you can see, some files and a folder were added to the project main directory. 'renv' folder includes the private library, and part of the infrastructure we mentioned earlier. 'renv.lock' file is the most important part of `renv` dependency management; it can be used to restore a previous state of the project, or to reinitialize the private library from scratch (useful when sharing the project).

You can see the current project private library in `renv/library/R-x.x/x86_64-w64mingw32` folder. Since our project is currently blank, only `renv` package itself was added to our library.

![Fig. 4: Initial private library](priv_lib1.PNG){fig-align="center"}
...
`tibble` and all of its dependencies are added to our private library.

![Fig. 5: Private library after installing tibble](priv_lib2.PNG){fig-align="center"}
...
We save this script in our project main folder, but you can also place it in a dedicated subfolder, as you like.

![Fig. 6: New script file](main_dir3.PNG){fig-align="center"}
...
`renv::init()` can also restore a previous state. If a lockfile already exists, running `renv::inits` prompts some choices:

![Fig. 7: init() with existing lockfile](init.PNG){fig-align="center"}
