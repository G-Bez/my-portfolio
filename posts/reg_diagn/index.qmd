...
These data points seem to have a somewhat odd behaviour. For example 'Porrentruy' has a relatively low 'Fertility' for such high 'Infant.Mortality' value, high 'Catholic' value and low 'Agriculture' value. 'Sierre' has a very high fertility even with a very low 'Education' rate. And so on for 'Rive Gauche' too.

Let's try to re-build the model without these 3 observation.
...
The 3 data points were actually very influencial. The new model is significantly more accurate, with higher *R^2^* and lower p-values (especially the 'Agriculture' and 'Infant.Mortality' ones got way lower). Now the linear model seems a way better fit, and we can effectively consider the 3 data points as outliers.
:::
